...
Object *ValuePointer::tag_as_object(address addr) {
  ASSERT_EQ(0u, reinterpret_cast<word>(addr) & kObjectTagMask);
...
Forwarder *ValuePointer::tag_as_forwarder(address addr) {
  ASSERT_EQ(0u, reinterpret_cast<word>(addr) & kForwarderTagMask);
...
Signal *ValuePointer::tag_as_signal(uword type, uword payload) {
  ASSERT_EQ(0u, type & ~kSignalTypeMask);
  ASSERT_EQ(0u, payload & ~kSignalPayloadMask);
